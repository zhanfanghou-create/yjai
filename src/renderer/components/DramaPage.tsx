...
- **文生视频提示词**：**必须**是可直接复制使用的完整**中文**提示词，**不包含文生图部分**。要求：
  ...
  3. 以「### 台词与声音」列出该镜的声音内容，必须包含三个小节：
     - 【台词】角色对白，每句一行，格式为 {对白内容}（大括号包裹，供引用与配音识别）；
     - 【旁白】旁白/画外音，格式为 {旁白内容}，无则写 {无}；
     - 【音效】环境音/动效描述（如雨声、脚步声、雷声），格式为 {音效描述}，无则写 {无}。
     台词必须从剧本原文中提取，不要改写、不要遗漏；台词旁白供配音和视频模型口型使用，务必完整。
  4. 以「### 约束词」保持人物身份、数量、服装、道具归属、空间方向和声音关系稳定。
  5. 画风统一：与影片调性一致（如 90年代中国农村电影），视频中不得出现任何字幕、文字叠加，纯画面，不要 bgm，不要配乐。
  6. 前后衔接（重要）：相邻镜头的画面必须保证**剧情完整、前后连贯顺畅**——前一镜结尾要自然承接后一镜开头，动作、走位、镜头视角、人物状态与时间线都要衔接得上，不得跳戏、断戏、情节矛盾或凭空切换场景/人物状态。
  7. 时长：每个镜头默认时长 15 秒左右（或 10 秒左右），**单镜不要过长**；在「画面描写」的每个镜头中标注该镜头时长。
