...
const callApi = async (c: APIConfig, n: AINode) => {
  ...
  if (imageModes.has(mode)) { if (win?.yijingAPI?.openai?.generate) { const r = await win.yijingAPI.openai.generate({ apiKey: c.apiKey, baseUrl: apiBase, ...imagePayload }); return { url: extractUrl(r), type: 'image' as const }; } const r = await fetch(`${apiBase}/images/generations`, { method: 'POST', headers: { Authorization: `Bearer ${c.apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify(imagePayload) }); const d = await r.json(); if (!r.ok) throw new Error(d?.error?.message || d?.error || `HTTP ${r.status}`); return { url: extractUrl(d), type: 'image' as const }; }
