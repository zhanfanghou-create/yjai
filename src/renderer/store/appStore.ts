...
interface AppState {
  ...
  apiConfigs: APIConfig[]; chatAPIConfigs: APIConfig[]; imageAPIConfigs: APIConfig[]; videoAPIConfigs: APIConfig[]; voiceAPIConfigs: APIConfig[]; musicAPIConfigs: APIConfig[]; comfyuiConfigs: ComfyUIConfig[]; recommendedConfigs: RecommendedConfig[]; modelscopeConfig: ModelScopeConfig | null; stockMediaSources: StockMediaSource[]; generationParams: GenerationParams; assistantSettings: AssistantSettings; comfyWorkflowCache: ComfyWorkflowCacheItem[];
  ...
  addRecommendedConfig: (c: Omit<RecommendedConfig, 'id'> & { id?: string }) => string; updateRecommendedConfig: (id: string, u: Partial<RecommendedConfig>) => void; deleteRecommendedConfig: (id: string) => void; testRecommendedConnection: (id: string) => Promise<TestConnectionResult>; fetchModelsForConfig: (id: string) => Promise<{ ok: boolean; models?: string[]; error?: string }>; clearAllRecommendedConfigs: () => void;
...
export const useAppStore = create<AppState>()(persist((set, get) => ({
  ...
  apiConfigs: [], chatAPIConfigs: [], imageAPIConfigs: [], videoAPIConfigs: [], voiceAPIConfigs: [], musicAPIConfigs: [], comfyuiConfigs: [], recommendedConfigs: [], generationParams: {}, assistantSettings: { voiceSource: 'edge-tts', speakReplies: false, nodeBackend: 'ask' }, comfyWorkflowCache: [],
  ...
  fetchModelsForConfig: async cid => { const s = get(); const c = [...s.apiConfigs, ...s.imageAPIConfigs, ...s.videoAPIConfigs, ...s.voiceAPIConfigs, ...s.musicAPIConfigs].find(x => x.id === cid); const r = s.recommendedConfigs.find(x => x.id === cid); if (r?.apiType === 'edge-tts') return { ok: true, models: EDGE_TTS_MODELS }; const cfg: any = c || r; if (!cfg) return { ok: false, error: '配置不存在' }; try { const win = window as any; let models: string[] = []; if (win?.yijingAPI?.grsai?.refreshModels) { const res = await win.yijingAPI.grsai.refreshModels({ baseUrl: cfg.baseUrl, apiKey: cfg.apiKey, apiType: 'openai-chat' }); if (Array.isArray(res?.models)) models = res.models.map(String); } if (!models.length) { const resp = await fetch(`${base(cfg.baseUrl)}/models`, { headers: cfg.apiKey ? { Authorization: `Bearer ${cfg.apiKey}` } : {} }); const data = await resp.json(); models = Array.isArray(data?.data) ? data.data.map((m: any) => m.id || m.name).filter(Boolean) : []; } return { ok: true, models }; } catch (e: any) { return { ok: false, error: e?.message || String(e) }; } }, clearAllRecommendedConfigs: () => set({ recommendedConfigs: [] }),
  ...
  partialize: s => ({ chatSessions: s.chatSessions, assets: s.assets, assetFolders: s.assetFolders, canvasHistory: s.canvasHistory, nodes: s.nodes, activeCanvasId: s.activeCanvasId, apiConfigs: s.apiConfigs, chatAPIConfigs: s.chatAPIConfigs, imageAPIConfigs: s.imageAPIConfigs, videoAPIConfigs: s.videoAPIConfigs, voiceAPIConfigs: s.voiceAPIConfigs, musicAPIConfigs: s.musicAPIConfigs, comfyuiConfigs: s.comfyuiConfigs, recommendedConfigs: s.recommendedConfigs, stockMediaSources: s.stockMediaSources, generationParams: s.generationParams, assistantSettings: s.assistantSettings, comfyWorkflowCache: s.comfyWorkflowCache, promptLibrary: s.promptLibrary, sidebarCollapsed: s.sidebarCollapsed, storyboardPresets: s.storyboardPresets, dramartProjects: s.dramartProjects, activeDramartId: s.activeDramartId, dramartCreateParams: s.dramartCreateParams, dramartCustomStyles: s.dramartCustomStyles, dramartDraft: s.dramartDraft }),
